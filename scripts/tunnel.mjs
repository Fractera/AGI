...
function scan(chunk) {
  ...
  if (match && !found) {
    ...
    log('адрес ВРЕМЕННЫЙ: перезапуск туннеля выдаст новый. Когда он протухнет, Cloudflare покажет')
    log('ошибку 1016 или 1033 — сайт при этом цел; человеку нужно вернуться в чат и попросить')
    log('агента обновить адрес (npm run serve:publish). Постоянный адрес — это свой домен.')
  }
