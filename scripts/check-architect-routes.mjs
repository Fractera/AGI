...
for (const path of onDisk) {
  ...
  // 🔒 ШАБЛОН СТРАНИЦЫ — ТРИ ЧАСТИ, И ПРОВЕРЯЮТСЯ ВСЕ ТРИ (255, решение владельца
  // 2026-09-19: «проверь, чтобы на всех страницах это было — я хочу создать из
  // этого переиспользуемый шаблон»).
  //
  // 🛑 ЗАБЫТАЯ ЧАСТЬ НЕ ЛОМАЕТ НИЧЕГО ВИДИМОГО, И ИМЕННО ПОЭТОМУ НУЖЕН СТОРОЖ.
  // Без `_components/` страница работает — просто нечему появиться, когда
  // содержимое напишут, и следующий агент сочтёт, что здесь другой уклад. Без
  // README он начнёт выяснять устройство по соседям. Без `generateMetadata`
  // страница отдаст заголовок всего сайта и, что хуже, останется индексируемой:
  // ✗ измерено до 255 — весь слой отвечал `robots: index, follow`.
  if (!existsSync(join(dir, '_components', 'index.ts'))) {
    fail(`«${path}»: нет _components/index.ts — странице неоткуда взять содержимое, когда его напишут`)
  }
  if (!existsSync(join(dir, 'README.md'))) {
    fail(`«${path}»: нет README.md — устройство страницы придётся выяснять по соседям`)
  }
  const pageSrc = existsSync(join(dir, 'page.tsx')) ? readFileSync(join(dir, 'page.tsx'), 'utf8') : ''
  if (!/collectionMetadata/.test(pageSrc)) {
    fail(`«${path}»: page.tsx не отдаёт generateMetadata — страница получит заголовок сайта и останется открытой поисковику`)
  }
