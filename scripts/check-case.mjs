...
//
// 🔒 ЧТО ЭТО ЛОВИТ — единственный класс, где Linux СТРОЖЕ Windows и macOS.
// Windows и macOS по умолчанию регистр в именах файлов не различают: импорт
// `@/lib/Showcase` при живом `lib/showcase.ts` работает здесь и на Маке, а на
// Linux сборка падает «module not found». Продукт ставится на домашние машины
// всех трёх систем, значит ошибка доедет до человека, которого мы не увидим.
//
