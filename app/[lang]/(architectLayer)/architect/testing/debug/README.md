# `/architect/testing/debug` — a section page
...
This section exists to show server-side state — environment, versions, what the
process actually sees. That makes it the single easiest place in the whole project
to leak a secret, and the leak would look like a feature.

**Values that must never be rendered here, in any mode:**

- API keys and tokens of any service (`*_API_KEY`, `*_TOKEN`, `*_SECRET`);
- database URLs and any string containing credentials;
- session secrets, signing keys, private keys;
- the contents of `.env.local` as a file.

**What is safe and actually useful:** which variables are SET (names only,
`present` / `missing`), Node and package versions, the port the process listens
on, the commit it was built from, uptime, and the last lines the process wrote.

🔒 **The rule, so it survives without this list:** print the ANSWER, never the
VALUE. "OPENAI_API_KEY: present" is a working diagnosis; the key itself is a
credential on a screen that may be shared, screenshotted, or opened on a temporary
public address. A debug page that shows a secret does not stop being a leak because
it is behind a lock — locks are removed, and screenshots outlive them.

🛑 **Masking is not an exception to this.** `sk-…abcd` still tells an attacker the
key exists, its shape, and its last characters; and masking code is exactly where
an off-by-one shows a few characters too many. Do not render the value at all.
