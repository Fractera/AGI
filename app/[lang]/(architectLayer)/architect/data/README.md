# `/architect/data` — a section page

One page of the architect layer. Everything about it lives in this folder; nothing
about it is written anywhere else.

It sits on the **first** level, next to the groups, and has no children yet. That is
a legal shape, not an unfinished one: the menu law says a group with no children
stays an ordinary item without a disclosure arrow, because there is nothing to open.
Add a folder next to this file and it becomes a group — see "Growing children".

🛑 **The folder is named `data` and the words folder is named `_data`.** They look
alike and mean different things: `data` is this section's address in the menu,
`_data` is the service folder Next never treats as a route because of the leading
underscore. Nothing collides — but read the import line twice before changing it.
...
product. Today this page is in state 3 by intent.
...
The moment a folder with its own `_data/` appears next to this file, the build
writes `_list.generated.ts` here and this page must become an index: swap
`CollectionPage` for `CollectionIndex` in `page.tsx` and pass it `PAGES` from that
generated list. Until then the list does not exist — the generator writes none for a
childless folder, and deletes a stale one — so importing it would break the build.
