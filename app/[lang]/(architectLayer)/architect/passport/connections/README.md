# `/architect/passport/connections` — a section page
...
## 🔒 Why this page is critical, and what it must NOT become

A node that cannot say who it depends on is a node nobody can safely replace. This
page answers three questions, in this order:

1. **Who does this node talk to** — name, address, and what it asks for;
2. **What does it promise back** — which of its own doors depend on that link;
3. **What stops working if the other side disappears** — named plainly, per link.

🛑 **The third question is the one that is always skipped, and it is the reason the
page exists.** A list of dependencies without consequences reads as decoration; the
person replacing a microservice needs to know what breaks, not what is connected.

🛑 **THIS IS NOT A REGISTRY OF EVERY SERVICE THAT EXISTS.** It lists what THIS node
uses. A copy of someone else's catalogue here would drift the moment the other side
changed, and drift silently — the failure mode this whole architecture is built to
avoid. What is known about a neighbour is asked of the neighbour, not remembered
here.

🔒 **A microservice is a replaceable block.** Whatever this node cannot do itself is
named here together with the address that can — an "it is not mine" without an
address is a dead end, and with one it is the next step.
