...
// 🔒 ХРАНИЛИЩЕ ПУТИ — СВОЙ ФАЙЛ В ДАННЫХ УЗЛА (274-2), А НЕ `.env.local` СЛОТА.
// В панели состояние пути лежало в `.env.local` гостевого приложения — там же, где языки, ключи и всё
// остальное; у файла было несколько писателей, и об этом сказано в законе ниже. В узле AGI слота нет, а
// данные узла живут в `data/`: файл принадлежит ТОЛЬКО пути запуска, и писатель у него один.
// 🛑 Формат оставлен прежним — строки `КЛЮЧ=значение`: перенесённый код читает и пишет его без правок.
const APP_ENV = process.env.APP_ENV_PATH ?? path.join(process.cwd(), "data", "node", "launch", "flow.env");
