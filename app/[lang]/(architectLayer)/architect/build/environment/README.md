# `/architect/build/environment` — a section page
...
## 🛑 Same rule as the debug page: print the ANSWER, never the VALUE

This page lists what the node reads from its environment. That makes it the second
place in the project where a secret can appear on a screen and look like a feature.

**Never render:** the value of any key, token, secret, password, or database URL —
not even masked. `sk-…abcd` still confirms the key exists, shows its shape, and
leaks its tail; and the masking code is exactly where an off-by-one shows more than
intended.

**Do render:** the NAME of each variable, whether it is `present` or `missing`,
what it is for in one line, and what stops working without it. That is a complete
diagnosis — it answers "why is this broken" without handing anyone the key.

🔒 A screen is shared, screenshotted, and sometimes opened on a temporary public
address. A lock does not protect a value that has already been photographed.
