...
const { agentServices, serviceDir } = require('../terminal/workspace.cjs')
...
function startIdlePoller() {
  ...
  globalThis[IDLE_KEY] = setInterval(async () => {
    if (busy) return
    busy = true
    try { for (const service of agentServices()) await idleTick(service) } catch (err) { console.error(`[telegram] узел не прочитал бота: ${err instanceof Error ? err.message : err}`) }
    busy = false
  }, IDLE_POLL_MS)
