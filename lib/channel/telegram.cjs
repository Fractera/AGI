...
//
// 🔒 ПОКА ТЕРМИНАЛ ВЫКЛЮЧЕН, БОТА ЧИТАЕТ УЗЕЛ (`startIdlePoller`, зовёт `server.js`). Он делает две вещи:
...
const { agentService } = require('../terminal/workspace.cjs')
const session = require('../terminal/session.cjs')
const { claudeAuthState, extraBins, resolveBin } = require('../terminal/claude-cli.cjs')
...
function terminalWithoutChannel() {
  const s = session.current()
  return Boolean(s && !s.channel)
}
 ...
 */
async function channelLaunch(env) {
  if (!storedToken() || access().allowed.length === 0) return null
  const pathKey = Object.keys(env).find((k) => k.toUpperCase() === 'PATH') || 'PATH'
  const dirs = [...extraBins(), ...String(env[pathKey] || '').split(path.delimiter).filter(Boolean)]
  const withBins = { ...env, [pathKey]: dirs.join(path.delimiter), TELEGRAM_STATE_DIR: stateDir() }
  const ready = await preparePlugin(withBins)
  if (!ready.ok) return { ok: false, error: ready.error }
  return {
    ok: true,
    env: withBins,
    args: ['--channels', `plugin:${PLUGIN}`, '--add-dir', stateDir(), '--permission-mode', 'auto'],
  }
}
...
function saveMessages(raw) {
  ...
  mkdirSync(stateDir(), { recursive: true })
  writeFileSync(messagesFile(), JSON.stringify(m, null, 2))
  return { ok: true }
}
...
async function statusMessage() {
  const m = { ...FALLBACK, ...readJson(messagesFile(), {}) }
  if ((await claudeSignedIn()) === false) return m.noSubscription
  return running() ? m.active : m.inactive
}

// ── УЗЕЛ ЧИТАЕТ БОТА, ПОКА ТЕРМИНАЛ ВЫКЛЮЧЕН ─────────────────────────────────

const IDLE_KEY = '__agiTelegramIdle'
...
async function idleTick() {
  ...
  for (const u of updates) {
    ...
    } else if (!access().allowed.includes(sender)) {
    ...
    await tg(token, 'sendMessage', { chat_id: msg.chat.id, text: await statusMessage(), disable_web_page_preview: true })
  }
}

/** Запускается один раз сервером узла (`server.js`), не дверью Next. */
function startIdlePoller() {
  if (globalThis[IDLE_KEY]) return
  let busy = false
  globalThis[IDLE_KEY] = setInterval(async () => {
    if (busy) return
    busy = true
    try { await idleTick() } catch (err) { console.error(`[telegram] узел не прочитал бота: ${err instanceof Error ? err.message : err}`) }
    busy = false
  }, IDLE_POLL_MS)
  globalThis[IDLE_KEY].unref?.()
}
...
function channelState() {
  ...
  return {
    ...
    running: running(),
    terminalWithoutChannel: terminalWithoutChannel(),
    stateDir: stateDir(),
...
module.exports = {
  PLUGIN, stateDir, storedToken, saveToken, activationLink, saveMessages, channelLaunch,
