...
async function checkActivation(greeting) {
  ...
  const text = String(greeting ?? '').slice(0, 1000)
...
// 🛑 МЕТКИ ЧУЖОЙ СЕССИИ CLAUDE CODE СНИМАЮТСЯ. ✗ Оплачено живым прогоном владельца 2026-09-22: pm2 и узел
// были подняты из рабочей сессии Claude Code и унаследовали `CLAUDECODE`, `CLAUDE_CODE_CHILD_SESSION`,
// `CLAUDE_CODE_SESSION_ID` и ещё семь меток. `claude` жителя, получив их, считал себя вложенной сессией
// («Transcript saving is off — inherited CLAUDE_CODE_CHILD_SESSION marker»), и сервер плагина Telegram на
// Bun не поднимался вовсе: процесса `bun.exe` нет, у бота 5 непрочитанных сообщений, pm2 пишет `online`.
// В памяти этого не было: там pm2 на сервере запущен вне всякой сессии Claude. `CLAUDE_CONFIG_DIR` и
// `CLAUDE_BIN` — настройки человека, их оставляем.
