...
let ephemeral = 0
/** Идёт запуск агента (ставятся зависимости плагина): второй запуск в это время отказывает, а не двоит сессию. */
let starting = false
...
function attachTerminal(server, app) {
  ...
  wss.on('connection', (ws) => {
     ...
     */
    async function startAgent(wantStart) {
      if (!pty) {
      ...
      if (!s) {
        ...
        clearTimeout(deadline)
        ws.send('\r\n[запускаю агента…]\r\n')
        const launch = await channelLaunch(env).catch((err) => ({ ok: false, error: err instanceof Error ? err.message : String(err) }))
        starting = false
        if (launch && !launch.ok) {
        ...
        if (ws.readyState !== ws.OPEN || session.current()) {
          fail('gone')
          return
        }
        let child
        ...
        s = session.register({ pid: child.pid, proc: child, cwd, channel: args.length > 0 })
        const born = s
        child.onData((data) => {
