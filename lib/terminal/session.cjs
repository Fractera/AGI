...
function register({ pid, proc, cwd }) {
  const session = { buffer: '', clients: new Set(), pid, proc, cwd, startedAt: new Date().toISOString() }
  box().session = session
  return session
}
...
function status() {
  ...
  return { running: true, pid: s.pid, cwd: s.cwd, clients: s.clients.size, startedAt: s.startedAt }
}
